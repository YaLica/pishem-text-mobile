...
/* ==========================================================================
   ...
   box-decoration-break: clone — то самое свойство, которым сделана подложкааа
...
async function exportNativeCanvas() {
  ...
  const clone = buildExportClone();
     // Холст создаётся размером width × height, снятым с живого узла. Клон внутри
  // SVG верстается заново с height:auto и выходит чуть короче — незакрытая
  // полоса снизу остаётся прозрачной. На белом фоне её не видно, на цветном
  // просмотрщик показывает её тёмной. Задаём точный размер: фон самой плашки
  // закрашивает полосу, скруглённые углы при этом сохраняются.
